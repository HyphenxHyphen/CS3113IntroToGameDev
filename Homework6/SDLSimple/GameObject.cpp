...
void GameObject::update(float deltaTime) {
	...
	for (GameObject* child : m_children) {
		child->setPosition(child->getPositionRelative() *  glm::vec3(getScale().y, getScale().x, getScale().z) + getPosition());
...
void const GameObject::setScale(XYZ xyz, float newVal) {
	m_scale[xyz] = newVal;
	for (GameObject* child : m_children) {
		child->setScale(child->getScaleRelative() * getScale());
		child->setPosition(child->getPositionRelative() * glm::vec3(getScale().y, getScale().x, getScale().z) + getPosition());
...
void const GameObject::addChild(GameObject* child) {
	m_children.push_back(child); 
	child->setParent(this);
	child->setPositionRelative(child->getPosition());
	child->setRotationRelative(child->getRotation());
	child->setScaleRelative(child->getScale());
}
