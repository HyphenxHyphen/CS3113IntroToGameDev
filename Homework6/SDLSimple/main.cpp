/**
* Author: John Li
* Assignment: Project Leaf Cutter
...
// Shader filepaths
const char V_SHADER_PATH[] = "shaders/vertex_colored_textured.glsl";
const char F_SHADER_PATH[] = "shaders/fragment_colored_textured.glsl";
...
void initialize()
{
	...
	g_gameGlobal.g_sfx["hit"] = Mix_LoadWAV("Assets/Audio/HitSFX.wav");
	Mix_VolumeChunk(
		g_gameGlobal.g_sfx["hit"],
		MIX_MAX_VOLUME * 50 / 100
	);

	g_gameGlobal.g_sfx["die"] = Mix_LoadWAV("Assets/Audio/DieSFX.wav");
	Mix_VolumeChunk(
...
void update() {
	...
	while (g_deltaTime >= FIXED_TIMESTEP)
	{
		...
		// Update Scene
		if ((switchSceneFlag == -1) && (!g_tfx->isRunning())) {
...
void shutdown() {
	for (int i = g_gameScenes.size() - 1; i >= 0; i--) {
		delete g_gameScenes[i];
	}
	g_gameScenes.clear();

	for (int i = g_gameGlobal.g_ants.size() - 1; i >= 0; i--) {
		if (g_gameGlobal.g_ants[i] != nullptr) {
			delete g_gameGlobal.g_ants[i];
		}
	}
	g_gameGlobal.g_ants.clear();

	//g_gameScenes.clear();
	g_gameScene = nullptr;
	for (auto& sfxPair : g_gameGlobal.g_sfx) {
