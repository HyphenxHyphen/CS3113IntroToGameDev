...
void Rigidbody2D::checkCollisions()
{
	for (auto gameObjectOther : *getAllGameObjects()) {
		if ((gameObjectOther != m_parent) && (gameObjectOther->isAlive())) {
			if (!ignoreCollisionTagsCheck(gameObjectOther)) {
				if (Rigidbody2D * rbOther = gameObjectOther->getComponent<Rigidbody2D>()) {
					if ((rbOther != this) && (rbOther->getPosition().z == getPosition().z)) {
						...
						if (collisionData.collided && (std::find(m_collisions.begin(), m_collisions.end(), rbOther) == m_collisions.end())) {
							//std::cout << getObjectID() << " at " << getPosition().x << ", " << getPosition().y << " entered collision with " << gameObjectOther->getObjectID() << " at " << gameObjectOther->getPosition().x << ", " << gameObjectOther->getPosition().y << std::endl;
	...
	for (auto collider : m_colliders) {
		for (auto colliderOther : rbOther->m_colliders) {
			...
			if (collisionColliderData.collided) {
				...
				//std::cout << getObjectID() << " at " << getPosition().z << ", " << getPosition().z << " entered collision with " << colliderOther->getObjectID() << " at " << colliderOther->getPosition().z << ", " << colliderOther->getPosition().z << std::endl;
...
bool Rigidbody2D::ignoreCollisionTagsCheck(GameObject* gameObjectOther) {
	for (auto tag : gameObjectOther->getTags()) {
		for (auto ignoreTag : m_ignoreTags) {
			if (tag == ignoreTag) {
				return true;
			}
		}
