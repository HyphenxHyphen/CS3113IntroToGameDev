...
void Scene3::initialise() {
	...
	tileMapLayer->addComponent<TileMap>(
		new TileMap(tileMapLayer, true, 51, 11,
			0, {}, 0,
	...
	//m_gameState.allGameObjects.push_back(tileMapLayer);
	tileMapLayer->addComponent<TileMap>(
		new TileMap(tileMapLayer, false, 51, 11,
			0,
	...
	// Sand
	tileMapLayer = new GameObject("TileMapSand0", &m_gameState.allGameObjects);
	//m_gameState.allGameObjects.push_back(tileMapLayer);
	tileMapLayer->addComponent<TileMap>(
		new TileMap(tileMapLayer, false, 51, 11,
			0, {}, 0,
	...
	player->addComponent<SpriteRenderer>(
		new SpriteRenderer(player, Util::loadTexture("Assets/Tiny Swords/Tiny Swords (Update 010)/Factions/Knights/Troops/Warrior/Blue/Warrior_Blue.png"), 6, 8, 0,
			...
			0, 12));
	player->setPosition({ 3.0f, -5.0f, 0.0f });
...
void Scene3::processInput(bool* gameIsRunning) {
...
void Scene3::processInputMainMenu(bool* gameIsRunning) {
	SDL_Event event;
...
void Scene3::processInputGameLevel(bool* gameIsRunning) {
	SDL_Event event;
	while (SDL_PollEvent(&event)) {
		if ((event.type == SDL_QUIT) || (event.type == SDL_WINDOWEVENT_CLOSE)) {
			*gameIsRunning = false;
...
void Scene3::processInputGameOver(bool* gameIsRunning) {
	SDL_Event event;
	while (SDL_PollEvent(&event)) {
		if ((event.type == SDL_QUIT) || (event.type == SDL_WINDOWEVENT_CLOSE)) {
			*gameIsRunning = false;
...
void Scene3::updateMainMenu(float deltaTime) {

}

void Scene3::updateGameLevel(float deltaTime) {
...
void Scene3::updateGameOver(float deltaTime) {

}

void Scene3::render(ShaderProgram* program) {
	...
	for (auto gameObject : m_gameState.allGameObjects) {
		if (gameObject->isAlive()) {
			if ((m_gameState.gameMode != MAIN_MENU) || (gameObject->getComponent<TileMap>())) {
	...
	for (auto& renderable : sortedRender) {
		//std::cout << "Rendering " << renderable->getObjectID() << " at elevation " << renderable->getPosition().z << std::endl;
	...
	if (m_gameState.gameMode == MAIN_MENU) {
		...
		Util::drawText(program, "PRESS ANY KEY TO GO TO MAIN MENU", glm::vec3(0.0f, -0.25f, 0.0f) + m_gameState.cameraPos, 1.5f, m_gameState.font.m_cols, m_gameState.font.m_rows, m_gameState.font.m_textureID);
	}
	else if ((m_gameState.gameMode == GAME_OVER) && (!m_gameState.victory)) {
