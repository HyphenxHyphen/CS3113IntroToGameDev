...
void resetGame() {
	...
	reference->setPosition(glm::vec3(3.5f, -6.5f, 1.0f));
	...
	// Irreg
	GameObject* irreg = new GameObject("Irregular Shape", &g_gameState.allGameObjects);
	...
	player->setPosition(glm::vec3(3.5f, -5.5f, 1.0f));
...
void updateGameLevel() {
	for (auto gameObject : g_gameState.allGameObjects) {
		if (gameObject->getObjectID() == "Irregular Shape") {
