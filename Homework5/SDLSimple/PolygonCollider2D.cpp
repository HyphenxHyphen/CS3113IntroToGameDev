...
void PolygonCollider2D::updateVertices() {
	for (unsigned int i = 0; i < m_verticesRelative.size(); i++) {
		...
		float newX = (cos(theta) * (m_vertices[i].x)) - (sin(theta) * (m_vertices[i].y));
		float newY = (sin(theta) * (m_vertices[i].x)) + (cos(theta) * (m_vertices[i].y));
		m_vertices[i].x = newX;
		m_vertices[i].y = newY;
